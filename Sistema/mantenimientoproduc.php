		...
		<nav id="navigation">
			...
			<div class="container">
				...
				<div id="responsive-nav">
				    ...
				    <li class="active"><a href="mantenimientoproduc.php">Mantenimientos de Productos</a></li>
...
$sql="Select * from nuevosproductos where estado ='A'";
$consulta=mysqli_query($conexion,$sql);

$totalfilas=mysqli_num_rows($consulta);
//echo "Total de nuevos clientes :".$totalfilas."<br>";
	echo "<tr>";
echo "<form action='nuevosproductos.php'>";
echo "<td><input type='submit' value='Registrar' class='btn btn-primary' name='btn_registrar'> Total de Nuevos Productos :".$totalfilas."<br></td>";
echo "</form><hr>";
	...
	echo "<table border='1' width='100%' height='450px'>";
					...
					echo "<tr>";
					echo "<td bgcolor='#FFFFFF'>id_Nproducto</center></td>";
					...
					//echo "<td bgcolor='#FFFFFF'><center> Opciones</center></td>";
...
while ($filas=mysqli_fetch_array($consulta)) {
	...
	echo "<td><img src='foto/".$filas['imagen']."'width=100' height='100' alt''</td>";
...
//consultar
$ver='window.open("consultarprod.php?id_Nproducto='.$filas['id_Nproducto'].'","Detalle", "width=1000, height=500,left=100,top=150,menubar=no,scrollbars=no,fullscreen=no,toolbar=no,status=no");';
echo"<td>&nbsp<a class='btn btn-success' href='#' onclick='".$ver."'>consultar</td>";



//actualizar
echo"<td><a class='btn btn-info' href='editarprod.php?id_Nproducto=".$filas['id_Nproducto']."'>Actualizar</td>";;


//eliminar
$eli='return confirm("Estas seguro que quieres eliminar este producto?")';
echo"<td>&nbsp<a class='btn btn-danger' href='eliminarprod.php?id_Nproducto=".$filas['id_Nproducto']."'onclick='".$eli."'>Eliminar</td>";
...
$totalfilas=mysqli_num_rows($consulta);
	echo "<tr>";
echo "<form action='nuevosproductos2.php'>";
echo "<td><input type='submit' value='Registrar' class='btn btn-primary' name='btn_registrar'> Total de Productos :".$totalfilas."<br></td>";
echo "</form><hr>";


	echo "<table class='table table-striped' border='1' width='100%' height='450px'>";
					echo "<tr><td colspan='17' bgcolor='#1E1F29' style='color:white' height='30'><center>LISTADO DE LA TABLA PRODUCTOS</center></td></tr>";
					echo "<tr>";
					echo "<td bgcolor='#FFFFFF'>Id</center></td>";
					echo "<td bgcolor='#FFFFFF'><center>Nombre Producto</center></td>";
					echo "<td bgcolor='#FFFFFF'><center>Existencia</center></td>";
...
while ($filas=mysqli_fetch_array($consulta)) {
	...
	echo "<td><img src='foto/".$filas['foto']."'width=100' height='100' alt''</td>";
...
//consultar
$ver='window.open("consultarprod2.php?id='.$filas['id'].'","Detalle", "width=1000, height=500,left=100,top=150,menubar=no,scrollbars=no,fullscreen=no,toolbar=no,status=no");';
echo"<td>&nbsp<a class='btn btn-success' href='#' onclick='".$ver."'>consultar</td>";



//actualizar
echo"<td><a class='btn btn-info' href='editarprod2.php?id=".$filas['id']."'>Actualizar</td>";;


//eliminar
$eli='return confirm("Estas seguro que quieres eliminar este producto?")';
echo"<td>&nbsp<a class='btn btn-danger' href='eliminarprod2.php?id=".$filas['id']."'onclick='".$eli."'>Eliminar</td>";
